...
const fileFilter = (req, file, cb) => {
  ...
  if ((isImage || isDocument) && mimetypeValid) {
    ...
    cb(new Error('Only images (jpeg, jpg, png, gif, webp) and documents (pdf, doc, docx, xls, xlsx, txt, xml, csv) are allowed!'), false);
  }
};
...
const logUploadedFiles = (req, res, next) => {
  if (req.files && req.files.length > 0) {
    ...
    console.log('📝 No images uploaded with this request');
  }
  next();
};
