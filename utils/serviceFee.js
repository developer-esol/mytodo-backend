...
const SERVICE_FEE_CONFIG = {
  ...
  CURRENCY_RATES: {
    'USD': 1,
    'AUD': 1.50,  // 1 USD = 1.50 AUD (approximate)
...
const getServiceFeeConfig = () => {
  return {
    ...
    minFees: Object.keys(SERVICE_FEE_CONFIG.CURRENCY_RATES).reduce((acc, currency) => {
      acc[currency] = SERVICE_FEE_CONFIG.MIN_FEE_USD * SERVICE_FEE_CONFIG.CURRENCY_RATES[currency];
      return acc;
    }, {}),
    maxFees: Object.keys(SERVICE_FEE_CONFIG.CURRENCY_RATES).reduce((acc, currency) => {
      acc[currency] = SERVICE_FEE_CONFIG.MAX_FEE_USD * SERVICE_FEE_CONFIG.CURRENCY_RATES[currency];
      return acc;
    }, {})
  };
};
...
const updateServiceFeeConfig = (newConfig) => {
  ...
  if (newConfig.CURRENCY_RATES !== undefined) {
    SERVICE_FEE_CONFIG.CURRENCY_RATES = { ...SERVICE_FEE_CONFIG.CURRENCY_RATES, ...newConfig.CURRENCY_RATES };
  }
  
  console.log('Service fee configuration updated:', SERVICE_FEE_CONFIG);
...
const validateServiceFeeCalculation = () => {
  console.log('=== Service Fee Calculation Examples ===');
  
  // Test cases
  const testCases = [
    { amount: 30, currency: 'USD', expected: { fee: 5, reason: 'minimum' } },
    { amount: 200, currency: 'USD', expected: { fee: 20, reason: 'percentage' } },
    ...
    { amount: 300, currency: 'AUD', expected: { fee: 30, reason: 'percentage' } },
  ...
  testCases.forEach((testCase, index) => {
    try {
      ...
      console.log(`Test ${index + 1}: ${testCase.currency} ${testCase.amount}`);
      console.log(`  Service Fee: ${result.currency} ${result.serviceFee} (${result.breakdown.reason})`);
      console.log(`  Total: ${result.currency} ${result.totalAmount}`);
