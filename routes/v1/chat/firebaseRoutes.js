...
router.post(
  "/chats/:taskId/messages",
  protect,
  validators.validateIndividualChatMessage,
  async (req, res) => {
    try {
      const chatId = `task_${req.params.taskId}`;
...
router.get(
  ...
  protect,
  validators.validateGetGroupMessages,
  async (req, res) => {
    try {
      const { taskId } = req.params;
      ...
      const hasAccess =
        groupChat.posterId.toString() === userId ||
...
router.post(
  "/group-chats/:taskId/messages",
  protect,
  validators.validateGroupChatMessage,
  async (req, res) => {
    try {
      const { taskId } = req.params;
      const { text, messageType = "text", metadata = {} } = req.body;
      ...
      if (!isTaskPoster && !isTasker) {
        return res.status(403).json({
          error:
            "Access denied. Only the task poster or users who have made offers can send messages to this group chat.",
        });
      }
      ...
      if (
        isTasker &&
        !groupChat.participants.find((p) => p.userId.toString() === senderId)
      ) {
      ...
      if (!hasAccess) {
        return res.status(403).json({
          error:
            "Access denied to send message in group chat. You must be the task poster or an active participant.",
        });
      }
      ...
      const messageData = {
        ...
        senderRole:
          groupChat.posterId.toString() === senderId ? "poster" : "tasker",
      };
...
router.post(
  ...
  validators.validateSystemMessage,
  async (req, res) => {
    try {
      const { taskId } = req.params;
      const { text, messageType, triggerUserId, metadata = {} } = req.body;
