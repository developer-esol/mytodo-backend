...
router.get("/", adminAuth, ...validators.getAnalytics, async (req, res) => {
  try {
    ...
    const userGrowth = userRegistrations.reduce(
    ...
    const userGrowthPercent =
      previousUsers > 0
        ? ((userGrowth - previousUsers) / previousUsers) * 100
        : 0;
    const taskGrowthPercent =
      previousTasks > 0
        ? ((taskGrowth - previousTasks) / previousTasks) * 100
        : 0;
    const revenueGrowthPercent =
      previousRevenue[0]?.total > 0
        ? ((summary.totalPayments - previousRevenue[0].total) /
            previousRevenue[0].total) *
          100
        : 0;
    const activeUserGrowthPercent =
    ...
    const assignedTasks =
      taskStatuses.find((t) => t._id === "assigned")?.count || 0;
    const completedTasks =
      taskStatuses.find((t) => t._id === "completed")?.count || 0;
    const cancelledTasks =
      taskStatuses.find((t) => t._id === "cancelled")?.count || 0;

    const totalTasksForCompletion =
      openTasks + assignedTasks + completedTasks + cancelledTasks;
    ...
    const averageOrderValue =
      summary.transactionCount > 0 ? summary.avgTransaction : 0;
      ...
      data: {
        ...
        monthlyTrends: monthlyRevenue.map((m) => ({
          ...
          newUsers: userRegistrations
            .filter(
              (u) => u._id.year === m._id.year && u._id.month === m._id.month
            )
            .reduce((sum, u) => sum + u.count, 0),
          tasksCreated: taskCreations
            .filter(
              (t) => t._id.year === m._id.year && t._id.month === m._id.month
            )
