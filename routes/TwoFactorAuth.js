    ...
    if (phone) {
      ...
      if (conflictingPendingUser) {
        ...
        if (conflictingPendingUser.otpExpires < new Date()) {
          ...
          return {
            error: "This phone number is currently being used for another registration. Please try again later or use a different phone number."
          };
        }
      }
    }
...
const sendOTP = async (email, phone, otp) => {
  try {
    ...
    if (phone && process.env.TWILIO_PHONE_NUMBER && process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
      try {
      ...
      console.warn(`SMS sending skipped for ${phone}: Twilio not properly configured`);
    }
...
router.post("/sms-verification", async (req, res) => {
  ...
  try {
    ...
    const existingUserByPhone = await User.findOne({ phone: pendingUser.phone });
  ...
  try {
    ...
    if (phone && phone !== pendingUser.phone) {
      ...
      if (existingPendingByPhone) {
        return res.status(400).json({
          success: false,
          message: "This phone number is currently being used for another registration."
        });
      }
  ...
  try {
    ...
    if (email) {
      ...
      results.email = {
        ...
        message: existingUser ? "Email is already registered" : 
                pendingUser ? "Email has pending verification" : "Email is available"
      };
    }
    ...
    if (phone) {
      ...
      results.phone = {
        ...
        message: existingUserByPhone ? "Phone number is already registered" :
                pendingUserByPhone ? "Phone number has pending verification" : "Phone number is available"
      };
    }
