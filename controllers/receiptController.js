...
const getMyReceipts = async (req, res) => {
  try {
    ...
    const transformedReceipts = receipts.map((receipt) => ({
      ...
      taskCategory:
        receipt.task?.categories?.[0] || receipt.taskDetails?.category,
      amount:
  ...
  try {
    ...
    const receiptDetails = {
      ...
      task: {
        ...
        location:
          receipt.task.location?.address || receipt.taskDetails.location,
...
const getTaskReceipts = async (req, res) => {
  try {
    ...
    if (!receipts || receipts.length === 0) {
      ...
      if (task.status === "completed") {
        ...
        try {
          ...
          const generatedReceipts = await generateReceiptsForCompletedTask(
            taskId
          );

          console.log(
            `✅ Generated receipts: Payment(${generatedReceipts.paymentReceipt.receiptNumber}), Earnings(${generatedReceipts.earningsReceipt.receiptNumber})`
          );
          ...
          if (newReceipts.length > 0) {
            ...
            const newPaymentReceipts = newReceipts.filter(
              (r) =>
                r.receiptType === "payment" &&
                r.poster.toString() === userId.toString()
            );
            if (newPaymentReceipts.length > 0) {
              userRelevantNewReceipts.push(newPaymentReceipts[0]); // Take the first one
              console.log(
                `📄 Added new payment receipt for poster: ${newPaymentReceipts[0].receiptNumber}`
              );
            }

            // Check if user is tasker (did the work)
            const newEarningsReceipts = newReceipts.filter(
              (r) =>
                r.receiptType === "earnings" &&
                r.tasker.toString() === userId.toString()
            );
            if (newEarningsReceipts.length > 0) {
              userRelevantNewReceipts.push(newEarningsReceipts[0]); // Take the first one
              console.log(
                `📄 Added new earnings receipt for tasker: ${newEarningsReceipts[0].receiptNumber}`
              );
            }

            const transformedReceipts = userRelevantNewReceipts.map(
              (receipt) => ({
                receiptId: receipt._id,
                receiptNumber: receipt.receiptNumber,
                receiptType: receipt.receiptType,
            ...
            console.log(
              `📤 Returning ${transformedReceipts.length} filtered newly generated receipts`
            );
          ...
          console.error(
            `❌ Failed to generate receipts for task ${taskId}:`,
            genError.message
          );
    ...
    console.log(
      `📤 Returning ${transformedReceipts.length} filtered receipts (removed duplicates)`
    );
