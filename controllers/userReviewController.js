  ...
  try {
    ...
    if (taskId) {
      ...
      if (task) {
        ...
        if (
          task.posterId &&
          task.posterId.toString() === reviewerId.toString()
        ) {
          reviewerRole = "poster";
        } else if (
          task.assignedTo &&
          task.assignedTo.toString() === reviewerId.toString()
        ) {
          reviewerRole = "tasker";
        }
      }
...
async function updateUserRatingStats(userId) {
  ...
  if (reviews.length > 0) {
    ...
    if (posterReviews.length > 0) {
      stats.asPoster.average = Number(
        (
          posterReviews.reduce((sum, r) => sum + r.rating, 0) /
          posterReviews.length
        ).toFixed(1)
      );
      stats.asPoster.count = posterReviews.length;
    }
    ...
    if (taskerReviews.length > 0) {
      stats.asTasker.average = Number(
        (
          taskerReviews.reduce((sum, r) => sum + r.rating, 0) /
